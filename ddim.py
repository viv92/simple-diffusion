### Program implementing Deep Denoising Diffusion Model (DDPM) - Ho et. al. wth DDIM sampling (deterministic sampling on subsequence of time steps with sigma = 0)
...
def sample2_ddim(net, alphas_hat, max_time_steps, subseq_steps, img_size, device, n):
    net.eval()
    with torch.no_grad():
...
if __name__ == '__main__':
    ...
    for ep in tqdm(range(max_epochs)):
        ...
        if ep % (max_epochs//100) == 0:
            ...
            sampled_img = sample2_ddim(net, alphas_hat, max_time_steps, subseq_steps, imgs.shape[-1], device, batch_size)
